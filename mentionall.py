...
async def start(event):
  await event.reply("**👻SoyguncularTaggerBot**\n **İle Grubunuzdakı Nerdeyse Tüm Üyelere Etiket Ata bilirim \nKomutlar için =======> /help yazın**",
                    buttons=(
                   
		      [Button.url('Beni Gruba Ekle ➕', f"https://t.me/Soyguncular_taggerbot?startgroup=a")],
                      [Button.url('İletişim', f"https://t.me/Hzchavo")],
                      [Button.url('Sahip', f"https://t.me/Moon_skkined_man")],
		      [Button.url('Grubumuz', 'https://t.me/SoyguncularTR')],
                    ),
                    link_preview=False
                   )
...
async def handler(event):
    ...
    if str(event.sender_id) not in SUDO_USERS:
        return await event.reply("__Sen sahibim değilsin Dostum !__")
    await event.reply('**Hey Bot Çalışıyor Merak Etme Sahip** \n Developer @Hzchavo')

@client.on(events.NewMessage(pattern='/durum'))
async def handler(event):
	
    await event.reply('**Tagger Bot un Durum Menüsü** \n Durum: Çalışıyorum✅ \n Telethon Sürümü: v1.24.0 \n Python Sürümü: v3.8+ \n Bot Sürümü: v0.2 \n Bu Botun Developeri @Hzchavo Dur')

@client.on(events.NewMessage(pattern='/bagis'))
async def handler(event):
	
    await event.reply('**Tagger Botun Bağış Menüsü** \n\nLink : Grubumuza Gelsen Yeterli Canım @SoyguncularTR \n\n  👻'

@client.on(events.NewMessage(pattern='/reklam'))
async def handler(event):
	
    await event.reply('__Botun Reklam Menüsü__\n**Reklam Veya İş Birliğimi Yapmak İstiyorsunuz Botun** [Sahibi](https://t.me/Hzchavo) **İle İletişime Geçiniz**')

print(">> Bot çalıyor merak etme 🚀 @Hzchavo bilgi alabilirsin <<")
